  ...
  fetch("https://pokeapi.co/api/v2/pokemon?limit=1000")
    ...
    .then(data => {
      ...
      result.forEach(pokemon => {
        // Buat elemen div untuk setiap Pokemon
        const item = document.createElement("div");
        item.setAttribute("class", item.name);
        item.innerHTML = pokemon.name
        // Panggil fungsi getImage dengan URL Pokemon
        getImage(pokemon.url, item);

        // Panggil fungsi getType dengan URL tipe Pokemon
        getType(pokemon.url, item);

        // Tambahkan elemen div ke dalam div "items"
...
function getImage(url, item) {
  fetch(url)
    ...
    .then(img => {
      ...
      // Tambahkan gambar sprite ke dalam elemen div yang sesuai
...
function getType(url, item) {
  fetch(url)
    ...
    .then(data => {
      console.log(data)
      // Ambil tipe Pokemon dari respons API
      const result = data.types
      // Buat elemen paragraf untuk menampilkan tipe
      const typeParagraph = document.createElement("p");
      typeParagraph.innerHTML = `Type: ${result[0].type.name}`;

      // Tambahkan elemen paragraf ke dalam elemen div yang sesuai
